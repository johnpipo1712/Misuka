...
(function ($) {
    ...
    $.FrameDialog.create = function (options) {
        try {
            ...
            var iframe = $("<iframe frameborder='0' scrolling='no' background='transparent' />")
                .attr("id", uid + "-VIEW")
                .attr("name", uid + "-VIEW")
                .css("margin", "0")
                .css("border", "0")
                ...
                .css("right", "0")
                .css("bottom", "0")
                .css("width", (width == 'auto' || width == undefined) ? "1900px" : width)
                .css("height", (height == 'auto' || height == undefined) ? "1200px" : height)
            ...
            //  - set the iframe src property here - this avoids the iframe's multiple-request issue 
                .bind('dialogopen', function (event, ui) {
                    ...
                    iframe.attr('src', url).load(function (e) {
                        if (width == 'auto' || width == undefined) {
                            $(this).contents().find('div.ui-inlinedialog-content .content').css("width", "auto");
                        }

                        if (height == 'auto' || height == undefined)
                            $(this).contents().find('div.ui-inlinedialog-content').css("height", "auto");
                ...
                .bind('dialogclose', function (event, ui) {
                    ...
                    window.setTimeout(
                        function () {
                            //cleanup the dialog if dialog instance is existing
                            if (frame.data('ui-dialog')) frame.dialog('destroy');
